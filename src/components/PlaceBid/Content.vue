<template>
  <nav class="container">
    <div id="content">
      <div class="contentleft">
        <p class="product">
          <u>{{product.fruit}}</u>
        </p>
        <div class="description">
          <p>{{product.description}}</p>
        </div>
        <div class="user">
          <img
            class="img"
            src="data:image/png;base64,"
          />
          <p class="username">{{product.user}}</p>
          <p class="star">★</p>
          <p class="point">{{product.point}}</p>
          <p class="timestart">Mở đấu giá từ {{product.timestart}}</p>
        </div>
        <div class="timeleft">
          <p class="time1">THỜI GIAN CÒN LẠI</p>
          <p class="time2">{{product.time2}}</p>
        </div>
        <div class="wannabid">
          <p class="bidnow">Bạn muốn đấu giá sản phẩm này chứ?</p>
          <b-button class="btn" type="is-success" rounded>Trả giá ngay</b-button>
        </div>
        <div class="info">
          <div class="user-rate">
            <p class="first-content">SẢN LƯỢNG</p>
            <p class="second-content">{{product.kg}} tấn</p>
          </div>
          <div class="user-quantity">
            <p class="first-content">GIÁ HIỆN TẠI</p>
            <p class="second-content">{{product.price}}đ</p>
          </div>
          <div class="user-time">
            <p class="first-content">BƯỚC GIÁ</p>
            <p class="second-content">{{product.step}}đ</p>
          </div>
        </div>
      </div>

      <div class="contentright">
            <b-carousel :indicator-inside="false">
        <b-carousel-item v-for="(item, i) in 8" :key="i">
            <span class="image">
              <img class="getimg" :src="getImgUrl(i)">
            </span>
        </b-carousel-item>
        <template slot="indicators" slot-scope="props">
            <span class="al image">
                <img :src="getImgUrl(props.i)" :title="props.i">
            </span>
        </template>
    </b-carousel>
      </div>
    </div>
    <br>
      <p>____________________________________________________________________________________________________________________________________________</p>

    <div id="nextcontent">
      <div class="contentleft1">
        <p class="productinfo">Thông tin sản phẩm</p>
        <div class="p1">
          <p class="ptext">VỊ TRÍ</p>
          <p class="ptext1">{{product.location}}</p>
        </div>
                <div class="p1">
          <p class="ptext">TỈ LỆ QUẢ (CHỪA CÀNH, LÁ) TRÊN TỔNG KHỐI LƯỢNG</p>
          <p class="ptext1">{{product.percent}}</p>
        </div>
                <div class="p1">
          <p class="ptext">NỒNG ĐỘ ĐƯỜNG</p>
          <p class="ptext1">{{product.sugardaddy}}</p>
        </div>
                <div class="p1">
          <p class="ptext">CÂN NẶNG TRUNG BÌNH MỖI QUẢ</p>
          <p class="ptext1">{{product.mean}}</p>
        </div>
                <div class="p1">
          <p class="ptext">ĐƯỜNG KÍNH TRUNG BÌNH MỖI QUẢ</p>
          <p class="ptext1">{{product.sugarmean}}</p>
        </div>
                <div class="p1">
          <p class="ptext">GIÁ KHỞI ĐIỂM</p>
          <p class="ptext1">{{product.startprice}}</p>
        </div>         
          <p class="ptext">THÔNG TIN CHI TIẾT</p>
          <div class="p2">
          <p class="ptext2">{{product.descriptiondetail}}</p>
        </div>
      </div>
      <div class="contentright1">
        <p class="productinfo">Ai đã đấu giá?</p>
        <div class="userpaidtime">
          <p class="ptext">TÊN NGƯỜI DÙNG</p>
          <p class="ptext">TRẢ GIÁ</p>
          <p class="ptext">THỜI GIAN</p>
        </div>
        <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>
                <div class="p3">
          <p class="ptext3">{{product.id}}</p>
          <p class="ptext">{{product.username}}</p>
          <p class="ptext">{{product.paid}}</p>
           <p class="ptext">{{product.time}}</p>          
        </div>

        <p class="seemore">XEM THÊM {{product.semo}} LƯỢT ĐẤU GIÁ NỮA</p>
      </div>
    </div>
  </nav>
</template>
🍎 Sản phẩm
<script>
export default {
      methods: {
      getImgUrl(value) {
          return `https://picsum.photos/id/43${value}/1230/500`
      }
    },
  name: "container",
  data() {
    return {
      product: {
        fruit: "🍎 Sản phẩm",
        description:
          "Táo này là táo Ambrossia nè các cậu ơi. Dòng thứ 2 của sản phẩm thì sẽ trông thế này còn đây là dòng thứ 3.",
        user: "Người dùng",
        point: "4.2",
        timestart: "12:00, 04/12/2019",
        time2: "10 ngày 23 tiếng 31 phút 38 giây",
        kg: "5.48",
        price: "80,000,000",
        step: "200,000",
        location: "Long An",
        percent: "97%",
        sugardaddy :"22.5%",
        mean: "40g",
        sugarmean: "8.5cm",
        startprice: "70,000,000đ",
        descriptiondetail: "Đây là thông tin chi tiết về sản phẩm. Nó có thể có nhiều dòng. Nhìn chung thì đây là phần mà mọi người có thể đọc hoặc không. Haiz gõ mệt quá.",
        id: "1",
        username: "*** ***** MANH",
        paid: "80,000,000đ",
        time: "12:50:39, 30/05/2020",
        semo: "10",
      },
    };
  },
};
</script>

<style scoped>
.container {
  /* background-color: rgba(255, 255, 255, 0.8); */
  /* background-color: blue; */
  backdrop-filter: saturate(180%) blur(20px);
  width: 1246px;
  height: 1121px;
  margin-top: 36px;
}
#content {
  display: flex;
}
.contentleft {
  backdrop-filter: saturate(180%) blur(20px);
  width: 603px;
  height: 500px;
}
.contentright {
  background-color: yellow;
  backdrop-filter: saturate(180%) blur(20px);
  width: 600px;
  height: 500px;
  margin-left: 43px;
}
#nextcontent {
  display: flex;
}
.contentleft1 {
  width: 605px;
  height: 428px;
  backdrop-filter: saturate(180%) blur(20px);
  margin-top: 40px;
}
.contentright1 {
  background-color: wheat;
  backdrop-filter: saturate(180%) blur(20px);
  width: 586px;
  height: 541px;
  margin-top: 40px;
  margin-left: 41px;
}
.product {
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.description {
  margin-top: 20px;
  width: 580px;
  height: 108px;
  font-family: "Merriweather";
  font-weight: 800;
  font-size: 28px;
  color: #01d28e;
}
.user {
  width: 179;
  height: 29;
  margin-top: 28px;
  display: flex;
}
.img {
  margin-top: 23px;
  width: 28px;
  height: 28px;
}
.username {
  margin-top: 23px;
  margin-left: 16px;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.star {
  margin-top: 23px;
  margin-left: 12px;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.point {
  margin-top: 23px;
  margin-left: 7px;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.timestart {
  margin-top: 23px;
  margin-left: 200px;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.timeleft {
  width: 364px;
  height: 64px;
  margin-left: auto;
  margin-right: auto;
  margin-top: 40px;
}
.time1 {
  text-align: center;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.time2 {
  text-align: center;
  font-family: "Roboto";
  font-weight: 500;
  font-size: 25px;
  color: #01d28e;
}
.wannabid {
  width: 522px;
  height: 44px;
  margin-top: 23px;
  margin-left: auto;
  margin-right: auto;
  display: flex;
}
.bidnow {
  font-family: "Roboto";
  font-weight: 400;
  font-size: 18px;
  color: #686868;
}
.btn {
  margin-top: -5px;
  margin-left: 30px;
  width: 182px;
  height: 44px;
}

.info {
  width: 502px;
  height: 64px;
  /* margin-left: auto;
  margin-right: auto; */
  margin-top: 35px;
  display: flex;
}
.user-rate {
  margin: 0 auto 0 50px;
  font-family: "Roboto";
  font-weight: 400;
  font-size: 15px;
  color: #686868;
  /* width: 232px; */
}
.user-quantity {
  margin: 0 auto 0 80px;
  font-family: "Roboto";
  font-weight: 400;
  font-size: 18px;
  color: #686868;
  /* width: 141px; */
}
.user-time {
  margin: 0 60px 0 100px;
  font-family: "Roboto";
  font-weight: 400;
  font-size: 18px;
  color: #686868;
  /* width: 84px; */
}
.first-content {
  width: 90px;
  margin-left: 15px;
  font-family: Roboto;
  font-size: 15px;
  color: #707070;
  opacity: 1;
}
.second-content {
  width: 120px;
  font-family: Roboto;
  font-size: 25px;
  color: #b88cd8;
  opacity: 1;
}
.is-active .al img {
    filter: grayscale(0%);
}
.al img {
    filter: grayscale(100%);
    /* margin-top: 190px; */
    height: 50px;
}
.getimg{
  height: 430px;
}
.productinfo{
  margin-bottom: 24px;
  font-family: "Merriweather";
  font-weight: 700;
  font-size: 25px;
  color: #B88CD8;
}
.p1{
  display: inline-flex;
  justify-content: space-between;
  width: 605px;
  height: 24px;
  margin-bottom: 16px;
}
.ptext{
  font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}
.ptext1{
  font-family: "Roboto";
  font-weight: 500;
  font-size: 20px;
  color: #01D28E;
}
.p2{
  margin-top:7px;
  width: 493px;
  height: 81px;
}
.ptext2{
  margin-top: 10px;
    font-family: "Roboto";
  font-weight: 500;
  font-size: 17px;
  color: #707070;
}
.userpaidtime{
  display: inline-flex;
  justify-content: space-between;
  width: 473px;
  height: 18px;
  margin-left: 55px;
  margin-bottom: 16px;
  background-color: blue;
}
.p3{
  display: inline-flex;
  justify-content: space-between;
  width: 586px;
  height: 18px;
  margin-bottom: 16px;
  background-color: blue;
}
  .ptext3{
  margin-right: -20px;
    font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
 
}
.seemore{
  margin-top:20px;
  text-align: center;
      font-family: "Roboto";
  font-weight: 500;
  font-size: 15px;
  color: #707070;
}

</style>
